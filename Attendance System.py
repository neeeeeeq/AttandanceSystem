...
def Take_image():
    ...
    degree=txt5.get()
#is_number(Id) and len(Id) != 0 and name.isalpha() and len(name)!=0 and  len(degree)!=0  and degree.isalpha()
    if((is_number(Id) and len(Id) != 0 and name.isalpha() and len(name)!=0 and  len(degree)!=0  and degree.isalpha())):#
        try:
            cursor.execute('''Exec  [dbo].[insert_student_data]
                            @sid = ?,
                            @snam = ?,
                            @Deg = ?''', Id, name , degree )
            con.commit()
        ...
        if (not is_number(Id)):
            res="Enter Only Numerics in Std no."
            message.configure(text=res)
        if (not name.isalpha()):
            res="Enter only Alphabets in Name"
            message.configure(text=res)
        if (not degree.isalpha()):
            ...
            if(conf < 80 ):
                ...
                try:
                    cursor.execute('''Exec  [dbo].[insert_student_data]
                        ...
                        @time = ?,
                        @date=?
                        ''', Id, std_nam , Degree,timeStamp,date)
                    con.commit()
                except:
